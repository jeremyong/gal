...
// An indeterminate encodes its degree in the monomial, as well as its identifier (if available).
// NOTE: "Degree" here is meant in the sense of a polynomial/monomial degree (e.g. x^2 has degree 2 and x*y^2*z has
// degree 3). We permit negative degrees to allow expressing linear combinations of nth-roots as well. The order of an
// indeterminate is the positive integer n such that g^k = 0 for all k >= n The dual unit in particular has order 2. An
...
// TRICK
// The shorter type names are intentional for producing less verbose type signatures in compile-errors and
// diagnostics.
struct mon
{
    rat q;
...
struct mon_it
{
    ...
    constexpr mon_it operator+(int delta) noexcept
    {
        return {ind_begin_, mon_ + delta};
    }
...
// T := Term capacity
// TRICK
// To restrict the number of template instantiations generated, we always restrict the array capacity to the next power
// of two.
...
//         }
//     }
// In the snippet above, dereferencing any of t, m, or g will result in the reference to the object in question.
...
struct mv
{
    ...
    constexpr void push(const_term_it it, rat scale, uint8_t e) noexcept
    {
...
namespace detail
{
    ...
    [[nodiscard]] constexpr auto sum(T1 const& lhs, T2 const& rhs) noexcept
    {
        constexpr size_t ind_size  = T1::ind_capacity() + T2::ind_capacity();
        constexpr size_t mon_size  = T1::mon_capacity() + T2::mon_capacity();
        constexpr size_t term_size = T1::term_capacity() + T2::term_capacity();
        mv<typename std::decay_t<decltype(lhs)>::algebra_t, ind_size, mon_size, term_size> out{};
            ...
            else if (rhs_it->element == lhs_it->element)
            {
                ...
                while (true) // Graded lexicographic comparison
                {
                    ...
                    else
                    {
                        ...
                        {
                            auto& it               = lhs_mon_it->degree < rhs_mon_it->degree ? lhs_mon_it : rhs_mon_it;
                            *out_mon_it            = *it;
                            out_mon_it->ind_offset = out_ind_it - out.inds.begin();
                            ++out_mon_it;
                            for (auto i_it = it.cbegin(); i_it != it.cend(); ++i_it)
                        ...
                        else
                        {
                            ...
                            while (true)
                            {
                                ...
                                {
                                    // Important! When adding, we adopt the lhs rational scaling factor so added
                                    // terms from the rhs need to be divided by the rhs rational scaling factor.
                                    ...
                                    if (!q.is_zero())
                                    {
                                        ...
                                        *out_mon_it            = *lhs_mon_it;
                                        ...
                                        for (auto i_it = lhs_mon_it.cbegin(); i_it != lhs_mon_it.cend(); ++i_it)
                                        {
                                            *out_ind_it++ = *i_it;
                                        }
                                    }
                                         ...
                                         mv_size& out_size) noexcept
    {
        ...
        for (auto term_it = start; term_it != end;)
        {
            ...
            for (; next != end; ++next)
            {
                ...
                {
                    for (auto mon_it = mon_start + next->mon_offset; mon_it != mon_start + next->mon_offset + next->count;
                         ++mon_it)
                    {
            ...
            sort(mon_cursor, out_mons_it, [ind_begin = ind_start](auto&& lhs, auto&& rhs) {
                return mon_view{lhs, ind_begin + lhs.ind_offset} < mon_view{rhs, ind_begin + rhs.ind_offset};
            });
            ...
            if (mon_count > 0)
            {
                *out_terms_it++ = term{mon_count, static_cast<width_t>(mon_cursor - out_mons_begin), term_it->element};
            }
    ...
    // Multiplication is always done left-to-right.
    // P := product operation between basis elements (returns a pair of a multiplier and target element)
    template <typename P, typename T1, typename T2>
    [[nodiscard]] constexpr auto product(P, T1 const& lhs, T2 const& rhs) noexcept
    {
        // The total number of terms conservatively is O(n*m) where n is the number of terms in the lhs and m is the
        // number of terms in the rhs. Note that this applies to both the number of indeterminates and the number of
        // monomials.
        constexpr width_t term_size = T1::term_capacity() * T2::term_capacity();
        constexpr width_t mon_size  = T1::mon_capacity() * T2::mon_capacity();
        constexpr width_t ind_size  = T1::mon_capacity() * T2::ind_capacity() + T2::mon_capacity() * T1::ind_capacity();

        // The monomials and indeterminates start out unsorted so we place them in temporary storage first before the
        // final sort-on-copy.
        ...
        // It is NOT the case that the product will move distinct term pairs to distinct term elements. For example, the
        // inner or contraction product can easily collapse multiple values to a scalar quantity.
        ...
        for (auto lhs_it = lhs.cbegin(); lhs_it != lhs.cend(); ++lhs_it)
        {
            for (auto rhs_it = rhs.cbegin(); rhs_it != rhs.cend(); ++rhs_it)
            {
                ...
                if (multiplier != 0)
                {
                    ...
                    // Multiply the polynomials of the lhs term and rhs term, scale it by the multiplier, and accumulate
                    // it into out. We do not bother to sort OR reduce as this will happen in the final pass.
                        ...
                        for (auto rhs_mon = rhs_it.cbegin(); rhs_mon != rhs_it.cend(); ++rhs_mon)
                        {
                            ...
                            while (true)
                            {
                                ...
                                else if (lhs_ind_it == lhs_ind_end || rhs_ind_it == rhs_ind_end)
                                {
                                    ...
                                    auto& it_end = lhs_ind_it == lhs_ind_end ? rhs_ind_end : lhs_ind_end;
                                    for (; it != it_end; ++it)
                                    {
                                        degree += it->degree;
                                        *temp_inds_it++ = *it;
                    ...
                    *temp_terms_it++ = term{mon_count, static_cast<width_t>(mon_cursor - temp_mons.begin()), element};
                }
            }
        }

        // The product between terms is not necessarily order-preserving so we need to both sort terms and monomials
        sort(temp_terms.begin(), temp_terms_it);

        mv<typename T1::algebra_t, ind_size, mon_size, term_size> out{};
    ...
    [[nodiscard]] constexpr auto extract(mv<A, I, M, T> const& in, std::array<uint8_t, N> const& elements) noexcept
    {
        mv<A, I, M, N> out{};
