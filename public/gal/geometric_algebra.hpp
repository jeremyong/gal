...
struct metric
{
    ...
    // The dot product between two 1-grade basis elements fully defines the Cayley table of an algebra.
    // The ordering convention here is degenerate elements are followed by positive norm elements which are finally
    // followed by negative norm elements.
...
struct algebra
{
    ...
    // At this point, non-diagonal metric tensors are not supported.
